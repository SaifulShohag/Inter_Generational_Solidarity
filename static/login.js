...
const uiRole  = params.get('role') || 'volunteer';
...
function setMode(m) {
  ...
  document.getElementById('consent-wrap').style.display = isReg ? 'flex' : 'none';
  ...
  if (!isReg) document.getElementById('consent-check').checked = false;
  clearStatus();
}
...
document.getElementById('open-tc').addEventListener('click',   () => tcModal.classList.add('open'));
document.getElementById('close-tc').addEventListener('click',  () => tcModal.classList.remove('open'));
tcModal.addEventListener('click', e => { if (e.target === tcModal) tcModal.classList.remove('open'); });
...
document.getElementById('auth-form').addEventListener('submit', async (e) => {
  ...
  if (mode === 'register' && !document.getElementById('consent-check').checked) {
    showStatus("Vous devez accepter les Conditions Générales d'Utilisation pour créer un compte.", 'error');
    return;
  }
